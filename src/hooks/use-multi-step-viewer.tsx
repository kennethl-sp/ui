...
export function MultiStepFormProvider({
  ...
  const goToNext = async () => {
    const currentStepData = steps[currentStepIndex - 1] as Stepfields;
  ...
  const goToStep = async (stepNumber: number) => {
    ...
    if (stepNumber > currentStepIndex && onStepValidation) {
      for (let i = currentStepIndex; i < stepNumber; i++) {
        const stepData = steps[i - 1] as Stepfields;
  ...
  const value: MultiStepFormContextType = {
    ...
    currentStepData: steps[currentStepIndex - 1] as Stepfields,
